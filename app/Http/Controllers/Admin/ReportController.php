...
class ReportController extends Controller
{
    ...
    function export_sale_report_excel(Request $request): \Symfony\Component\HttpFoundation\StreamedResponse|string
    {
      $query_param = [];
      $branches = $this->branch->all();
      $branch_id = $request['branch_id'];
    ...
    foreach ($this->order_detail->whereIn('order_id', $orders)->get() as $detail) {
        ...
        $exportData[] = [
            'Product info' => $product['name'] . ' - ' . $product['weight'],
            'Qty' => $detail['quantity'],
            'Date' => $detail['created_at']->format('d M Y'),
            'Amount' => number_format($ord_total, 2),
        ];
        ...
        // Total orders is the count of unique product IDs
         $total_order = count($orders);

        // Add totals to the top of the export data
        $exportData = array_merge([
         [
            'Product info' => 'Total item qty',
            'Qty' => $total_qty,
            'Date' => '',
            'Amount' => number_format($total_sold, 2),
        ],
        [
            'Product info' => 'Total Orders',
            'Qty' => $total_order,
            'Date' => '',
            'Amount' => '',
        ],
        ['Product info' => '', 'Qty' => '', 'Date' => '', 'Amount' => '']
        ], $exportData);

         // Export to Excel
        return (new FastExcel($exportData))->download('sale_report.xlsx');
       }
    /**
     * @param Request $request
     * @return Application|Factory|\Illuminate\Contracts\View\View
