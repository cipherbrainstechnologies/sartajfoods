...
<body style="-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important;">
	<table width="100%" class="invoice_table">
		<tr style="background: #3e4493;">
			<td>
				<h2 style="margin: 10px 0;color: #fff;text-align: center;font-size: 15px;">{{translate('INVOICE')}}</h2>
			</td>
		</tr>
		<tr>
			<td>
				...
				<table width="100%">
					<tr>
						<td width="50%">
							...
							<p style="border-bottom: 2px solid #3e4493;margin: 0; padding: 0 0 10px;"><b>{{translate('Tel')}} </b>
                                {{$order->delivery_address['contact_person_number']}}
                            </p>
							<table width="100%">
								<tr>
									<td width="70%"><h3 style="margin: 0;">{{translate('Total Payable Amount')}}</h3></td>
						...
						<td width="50%" style="padding-left: 30px;">
							...
							<p style="margin:0"><b>{{translate('Tel')}}</b>{{$order->shop_detail['phone']}}</p>
			...
			<td>
				<table width="100%" class="invoice_table_product">
					<tr style="background: #a0a3ca;">
						<th width="50%" style="text-align: center;">{{translate('Product Name')}}</th>
						<th width="10%" style="text-align: center;">{{translate('Qty.')}}</th>
						<th width="20%" style="text-align: center;">{{translate('Price Per pc.')}}</th>
						<th width="20%" style="text-align: center;">{{translate('Total Amount')}}</th>
					</tr>
						...
						<td width="50%">
							<table width="100%" class="invoice_table_info">
                                ...
                                @if($totalTaxPercent['TotalTenPercentTax']!=0)
                                    <tr>
                                        <td width="55%">{{translate('10% Tax')}}</td>
                                        <td width="5%">¥</td>
                                ...
                                @if($EightPercentTax != 0)
                                    <tr>
                                        <td width="55%">{{translate('Consumption Tax 8%')}}</td>
                                        <td width="5%">¥</td>
                                ...
                                @if($TenPercentTax != 0)
                                    <tr>
                                        <td width="55%">{{translate('Consumption Tax 10%')}}</td>
                                        <td width="5%">¥</td>
                                ...
                                @if($totalDiscount != 0)
                                    <tr>
                                        <td width="55%">{{translate('Discount Amount')}}</td>
                                        <td width="5%">¥</td>
                                ...
                                @if($order->delivery_charge!=0)
                                    <tr>
                                        <td width="55%">{{translate('Delivery Charge')}}</td>
                                        <td width="5%">¥</td>
                                ...
                                @endif
								<tr>
									<td width="55%"><h3 style="margin: 0;">{{translate('Total')}}</h3></td>
